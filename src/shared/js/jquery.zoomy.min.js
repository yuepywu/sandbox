...
(function ($) {
    ...
    zoomy.prototype = {
        ...
        init: function (e, config) {
            ...
            //Setup Vars 
            this.body = $('html body');
            this.popoverHtml = '<div class="popover ' + this.settings.popoverClassName + '"><div class="arrow"></div><div class="popover-content"><img class=' + this.settings.popoverClassName + '_image' + ' src="" /></div></div>';
            this.sandboxHtml = '<div class="popover ' + this.settings.popoverClassName + '_sandbox' + '"><div class="arrow"></div><div class="popover-content"></div></div>';
            ...
            this.e.attr({
                'data-zoomed-image': this.zoomedImageSource + '?' + this.id
            });
        ...
        updateImage: function() {
            ...
            //this.newZoomedImageSource = this.e.attr('data-zoomed-image').split('?')[0];
            /*
            console.log(img + '?' + this.id);
            this.e.attr({
                'data-zoomed-image': img + '?' + this.id
            });
            this.zoomedImageSource = this.e.attr('data-zoomed-image');
        ...
        grabData: function() 
        {
            ...
            if (this.settings.innerZoom) { // appends the large image to lens if the option is true
                ...
                if (zoomedImageSource !== this.popover.find('img').attr('src')) {
                    this.popover.find('.popover-content').html(el.clone());
                }
            }
        ...
        {   
            ...
            if (!this.settings.innerZoom){
                var self = this,
                    ...
                    element = this.thumb,
                    popover = this.popover,
                    popover_inner = this.popover.find('.popover-content');


                var popoverTop, popoverLeft, breakPointRight;

                switch (this.settings.popoverPosition) {
                case 'auto':

                	popoverTop = element.offset().top - ((this.settings.popoverHeight - element.height())*0.5);
                    popoverLeft = element.offset().left + element.outerWidth() + this.settings.popoverOffsetX;
                	breakPointRight = element.offset().left + element.outerWidth() + this.settings.popoverWidth;
                    $(popover).removeClass('left').addClass('right'); 
                	if (breakPointRight > $(window).width()) {
                		popoverTop = element.offset().top - ((this.settings.popoverHeight - element.height())*0.5);
                ...
                popover_inner.css({
                    height: this.popoverHeight - 20,
                    width: this.popoverWidth - 20
                });
    ...
    $.fn.updateImage = function () {
        $(this).get(0).zoomy.updateImage();
    },
